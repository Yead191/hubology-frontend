...
import { hourlyRateOptions } from "@/lib/validators";
...
/** Rank of a rate band for sorting (index in the canonical order). */
function rateRank(rate: string) {
  const i = hourlyRateOptions.indexOf(rate as (typeof hourlyRateOptions)[number]);
  ...
  const vendors = React.useMemo(() => {
    ...
    switch (filters.sort) {
      case "reviews":
        ...
        break;
      case "rate-asc":
        sorted.sort((a, b) => rateRank(a.hourlyRate) - rateRank(b.hourlyRate));
        break;
      case "rate-desc":
        sorted.sort((a, b) => rateRank(b.hourlyRate) - rateRank(a.hourlyRate));
        break;
      default:
      ...
      <div className="relative mx-auto max-w-6xl px-4 sm:px-6">
        ...
        <header className="max-w-2xl">
          ...
          <p className="mt-3 text-pretty text-mist">
            Every expert is manually reviewed. Search, filter, and find the right
            person — then reach out directly by phone or email.
          </p>
        </header>
