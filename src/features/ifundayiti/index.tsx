...
import { Settings, RefreshCw, CheckCircle, ShieldAlert, Award, Trophy, Users, Eye, HelpCircle } from "lucide-react";
import { IFundAyitiProvider, useIFundAyiti } from "./context/ifundayiti-context";
...
function IFundAyitiContent() {
  ...
  return (
    <div className="relative min-h-screen bg-ink text-cloud">
      ...
      {/* Senior Developer Flow Simulator Control Panel (Sticky at bottom right or expandable banner) */}
      <div className="fixed bottom-6 right-6 z-90 max-w-sm sm:max-w-md print:hidden">
        <div className={`border-gradient rounded-3xl bg-panel-soft/95 backdrop-blur-md shadow-2xl transition-all duration-300 border-hairline-strong ${simOpen ? "p-6 h-h-100 overflow-y-auto" : "p-3 h-auto"
          }`}>
          {/* Header */}
                ...
                <div className="flex items-center justify-between gap-4">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className={`px-2 py-1 rounded text-[10px] font-bold ${period.status === "Open" ? "bg-emerald-500 text-white" : "bg-white/5 text-mist hover:text-cloud"
                        }`}
                    >
                      Open
                    </button>
                    <button
                      ...
                      className={`px-2 py-1 rounded text-[10px] font-bold ${period.status === "Closed" ? "bg-amber-500 text-white" : "bg-white/5 text-mist hover:text-cloud"
                        }`}
                    >
                      Closed
                    </button>
                      ...
                      <div key={app.id} className="flex items-center justify-between gap-2 p-2 bg-ink/40 rounded border border-hairline/50">
                        ...
                        <div className="flex gap-1.5 shrink-0">
                          <button
                            ...
                            className="bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 px-1.5 py-0.5 rounded text-[9px] font-bold hover:bg-emerald-500 hover:text-white"
                          >
                            Approve
                          </button>
                          <button
                            onClick={() => simRejectApplicant(app.id)}
                            className="bg-rose-500/20 text-rose-300 border border-rose-500/30 px-1.5 py-0.5 rounded text-[9px] font-bold hover:bg-rose-500 hover:text-white"
                          >
                            Reject
                          </button>
                        </div>
                    ...
                    {approvedApps.map(app => (
                      <div key={app.id} className="flex items-center justify-between gap-2 p-2 bg-ink/40 rounded border border-hairline/50">
                        ...
                        <button
                          ...
                          className="bg-violet/30 text-violet-bright border border-violet/40 px-2 py-0.5 rounded text-[9px] font-bold hover:bg-violet-bright hover:text-white shrink-0"
                        >
                          Select Finalist
                        </button>
                      </div>
